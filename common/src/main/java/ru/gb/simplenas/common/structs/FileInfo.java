...
public class FileInfo implements Serializable {
    ...
    @Override public String toString () {
        String tm = (directory) ? "-" : FileTime.from(modified, CommonData.filetimeUnits).toString(), tc = (directory) ? "-" : FileTime.from(created, CommonData.filetimeUnits).toString();

        String name = fileName;
        if (name == null) { name = "null"; }
        return String.format(FORMAT_FILEINFO, name, directory ? 'D' : 'F', exists ? 'E' : '-', symbolic ? 'S' : '-', filesize, tm, tc);
    }
