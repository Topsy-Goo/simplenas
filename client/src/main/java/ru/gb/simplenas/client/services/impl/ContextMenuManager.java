        ...
        if (menu != null && tv != null) {
            menu.setOnShowing(new EventHandler<WindowEvent>() {
                @Override public void handle (WindowEvent event) {
                    ...
                    boolean noselection = tfi == null;
                    boolean dir = !noselection && tfi.getFolder();

                    ObservableList<MenuItem> list = menu.getItems();
                    for (MenuItem mi : list) {
                        String ud = (String) mi.getUserData();
                        if (ud != null) {
                            if (ud.equals("dir")) { mi.setDisable(noselection || !dir); }

                            if (ud.equals("file")) { mi.setDisable(noselection || dir); }

                            if (ud.equals("any")) { mi.setDisable(noselection); }
                        }
                    }
                }
            });
